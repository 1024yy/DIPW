from lhhloader2 import myDataset
...
psnr_w1 = np.zeros((batch_size, 3))
psnr_w2 = np.zeros((batch_size, 3))
ssim_c = np.zeros(batch_size)
ssim_w1 = np.zeros(batch_size)
ssim_w2 = np.zeros(batch_size)
lpipc_sum = []
diffC_sum = []
psnr_c_sum = []
ssim_c_sum = []
...
for patch, cover, watermark, box, cover_name, water_name in train_loader:
    ...
    with torch.no_grad():
        ...
        container_patch1 = itm_secret_img + patch1
        container_patch2 = itm_secret_img + patch2

        container_patch_numpy1 = container_patch1.clone().cpu().detach().numpy()
        container_patch_numpy1 = container_patch_numpy1.transpose(0, 2, 3, 1)

        container_patch_numpy2 = container_patch2.clone().cpu().detach().numpy()
        container_patch_numpy2 = container_patch_numpy2.transpose(0, 2, 3, 1)
        ...
        N, _, _, _ = container_patch1.shape
        import PerceptualSimilarity.models
        ...
        for B1 in range(batch_size):
            # for ii in range(box[0].shape[0]):
            box_p0 = box[0][0]
            box_p1 = box[0][1]
            cover_numpy[B1][box_p0[0]:box_p0[1], box_p0[2]:box_p0[3]] = container_patch_numpy1[B1]
            cover_numpy[B1][box_p1[0]:box_p1[1], box_p1[2]:box_p1[3]] = container_patch_numpy2[B1]
...
for patch_r, cover_r, watermark_r, box_r, cover_r_name, water_r_name in test_loader:
    Rnet.eval()
    with torch.no_grad():
        patch_r1 = patch_r[0].cuda()
        patch_r2 = patch_r[1].cuda()
        watermark_r = watermark_r.cuda()
        rev_secret_img1 = Rnet(patch_r1)
        rev_secret_img2 = Rnet(patch_r2)
        ...
        rev_secret_img_numpy1 = rev_secret_img1.clone().cpu().detach().numpy()
        rev_secret_img_numpy1 = rev_secret_img_numpy1.transpose(0, 2, 3, 1)

        rev_secret_img_numpy2 = rev_secret_img2.clone().cpu().detach().numpy()
        rev_secret_img_numpy2 = rev_secret_img_numpy2.transpose(0, 2, 3, 1)
        ...
        for i1 in range(batch_size):
            ...
            rev_secret_img_show1 = cv2.cvtColor(rev_secret_img_numpy1[i1], cv2.COLOR_RGB2BGR)
            rev_secret_img_show2 = cv2.cvtColor(rev_secret_img_numpy2[i1], cv2.COLOR_RGB2BGR)
            # cv2.imshow('1', rev_secret_img_show1)
            # cv2.imshow('2', rev_secret_img_show2)
            # cv2.waitKey()
            print(water_r_name_str + '.png')
            cv2.imwrite(os.path.join(rev_dir, water_r_name_str + '1.png'), rev_secret_img_show1 * 255)
            cv2.imwrite(os.path.join(rev_dir, water_r_name_str + '2.png'), rev_secret_img_show2 * 255)
            psnr_w1[i1, 0] = PSNR(rev_secret_img_numpy1[i1, :, :, 0], watermark_r_numpy[i1, :, :, 0])
            psnr_w1[i1, 1] = PSNR(rev_secret_img_numpy1[i1, :, :, 1], watermark_r_numpy[i1, :, :, 1])
            psnr_w1[i1, 2] = PSNR(rev_secret_img_numpy1[i1, :, :, 2], watermark_r_numpy[i1, :, :, 2])

            psnr_w2[i1, 0] = PSNR(rev_secret_img_numpy2[i1, :, :, 0], watermark_r_numpy[i1, :, :, 0])
            psnr_w2[i1, 1] = PSNR(rev_secret_img_numpy2[i1, :, :, 1], watermark_r_numpy[i1, :, :, 1])
            psnr_w2[i1, 2] = PSNR(rev_secret_img_numpy2[i1, :, :, 2], watermark_r_numpy[i1, :, :, 2])

            ssim_w1[i1] = SSIM(rev_secret_img_numpy1[i1], watermark_r_numpy[i1], multichannel=True)
            ssim_w2[i1] = SSIM(rev_secret_img_numpy2[i1], watermark_r_numpy[i1], multichannel=True)

        psnr_w_sum1.append(psnr_w1.mean().item())
        psnr_w_sum2.append(psnr_w2.mean().item())
        ssim_w_sum1.append(ssim_w1.mean().item())
        ssim_w_sum2.append(ssim_w2.mean().item())
        print("Avg. PSNR W:", psnr_w1.mean().item())
        print("Avg. PSNR W:", psnr_w2.mean().item())
        print("Avg. SSIM W:", ssim_w1.mean().item())
        print("Avg. SSIM W:", ssim_w2.mean().item())
...
avg_psnr_w1 = np.mean(np.array(psnr_w_sum1))
avg_psnr_w2 = np.mean(np.array(psnr_w_sum2))
avg_ssim_w1 = np.mean(np.array(ssim_w_sum1))
avg_ssim_w2 = np.mean(np.array(ssim_w_sum2))
...
print('|Average PSNR W1 %5f|' % avg_psnr_w1)
print('|Average PSNR W2 %5f|' % avg_psnr_w2)
print('|Average SSIM W1 %5f|' % avg_ssim_w1)
print('|Average SSIM W2 %5f|' % avg_ssim_w2)
